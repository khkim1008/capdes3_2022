...
def run():
    ...
    fn1 = 'D:/2022-2학기/캡스톤디자인/new/사용자유형별_정류소_이용현황.csv' #2019년 버스탑승 데이터 읽기
    f1 = pd.read_csv(fn1, encoding='cp949')
#f1 = pd.read_excel(fn1) 
    f1.info()  
    
#2020년 4월부터 2021년 9월까지 탑승 데이터 읽어서 2019 데이터와 합치기  ---------------           
    start_month = 3
    end_month = 9
    year = 2020
    k = 0    
    ...
    for i in range(0, 18):
        ...
        if i > 8 :
            ...
            enc = 'cp949'
        name = 'Results_' + str(year+k) + month
        print(i, name)
        dirname = 'D:/2022-2학기/캡스톤디자인/new/2000_2001/'
        tf = pd.read_csv(dirname+name+'.csv', encoding=enc)
        tf = tf[['base_date', 'bus_station_name', 'user_category', 'user_count']]
        tf.rename(columns = {'bus_station_name':'station_name', 'user_category':'user_type' }, inplace=True)
        f1 = pd.concat([f1, tf])    
    ...
    sn = f1['station_name'].str.strip()
    f1["station_name"] = sn
    정류소별_탑승객수 = f1.groupby(['station_name'])['user_count'].sum()
    #정류소별_탑승객수 = f1.groupby(['sn'])['user_count'].sum()
    정류소별_탑승객수.info()
